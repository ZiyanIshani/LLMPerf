...
async def send_request(
    client, prompt, output_tokens, model, request_timeout, force_output_tokens=False
) -> RequestMeasurement:
    ...
    async def _stream_request():
        ...
        if force_output_tokens:
            # Backend-specific: not all OpenAI-compatible servers honor this
            # (notably Ollama's compat layer does not) — natural EOS-stop
            # still applies there even with this flag set.
            create_kwargs["extra_body"] = {"ignore_eos": True}
    ...
    req.actual_tokens = actual_tokens
    ...
    async def bounded_request():
        ...
        async with semaphore:
            return await send_request(
                client, prompt, output_tokens, model, request_timeout, force_output_tokens
            )
...
async def run_warmup(
    client, prompts, output_tokens, model, warmup_requests, request_timeout, force_output_tokens=False
):
